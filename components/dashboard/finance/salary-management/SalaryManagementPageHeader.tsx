...
interface SalaryManagementPageHeaderProps {
  onAddSalary: () => void;
  onImportSalaries: () => void;
  onMoreOptions: () => void;
}

export default function SalaryManagementPageHeader({
  onAddSalary,
  onImportSalaries,
  onMoreOptions,
}: SalaryManagementPageHeaderProps) {
  return (
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
      <div>
        ...
        <nav className="flex items-center gap-1.5 mt-1 text-xs">
          ...
          <ChevronRight className="h-3 w-3 text-slate-400" />
          <span className="text-slate-500 font-medium">Salary Management</span>
        </nav>
      </div>
