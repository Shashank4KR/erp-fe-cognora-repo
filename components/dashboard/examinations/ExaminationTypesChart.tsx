...
import { EXAM_TYPES_DATA } from "@/lib/fixtures/examinations-reference-fixture";

interface ExaminationTypesChartProps {
  onViewAll?: () => void;
}

export default function ExaminationTypesChart({ onViewAll }: ExaminationTypesChartProps) {
  const total = EXAM_TYPES_DATA.reduce((sum, item) => sum + item.value, 0);
      ...
      <div className="flex items-center gap-6">
        <div className="flex-shrink-0">
          <svg width="120" height="120" viewBox="0 0 120 120">
            {EXAM_TYPES_DATA.map((segment, i) => {
              const radius = 48;
              const circumference = 2 * Math.PI * radius;
              const percentage = segment.value / total;
              const dashArrayLength = percentage * circumference;
              const prevDashOffset = EXAM_TYPES_DATA.slice(0, i).reduce((sum, s) => sum + (s.value / total) * circumference, 0);
              return (
                <circle
                  key={segment.label}
                  cx={60}
