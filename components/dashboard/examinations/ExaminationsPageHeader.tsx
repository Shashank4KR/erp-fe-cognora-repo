...
interface ExaminationsPageHeaderProps {
  onCreateExamination: () => void;
  onMoreOptions: () => void;
}

export default function ExaminationsPageHeader({
  onCreateExamination,
  onMoreOptions,
}: ExaminationsPageHeaderProps) {
  return (
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
      <div>
        ...
        <nav className="flex items-center gap-1.5 mt-1 text-xs">
          ...
          <span className="text-slate-500 font-medium">Examinations</span>
        </nav>
      </div>
      <div className="flex items-center gap-2 mt-4 sm:mt-0">
        <button
          onClick={onCreateExamination}
