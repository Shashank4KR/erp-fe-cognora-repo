...
import { Plus, CalendarDays, MoreVertical, ChevronRight } from "lucide-react";
import { Button } from "@/components/ui/button";

interface MessManagementPageHeaderProps {
  onAddClick: () => void;
  onViewMenuPlan: () => void;
  onMoreOptions: () => void;
}

export default function MessManagementPageHeader({
  onAddClick,
  onViewMenuPlan,
  onMoreOptions,
}: MessManagementPageHeaderProps) {
  return (
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
      <div>
        ...
        <nav className="flex items-center gap-1.5 mt-1 text-sm" aria-label="Breadcrumb">
          ...
          <ChevronRight className="w-3.5 h-3.5 text-slate-400" />
          <span className="text-slate-500">Mess Management</span>
        </nav>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <button
          type="button"
          onClick={onViewMenuPlan}
          className="inline-flex items-center gap-2 rounded-lg border border-purple-200 bg-white px-4 py-2 text-sm font-semibold text-[#7c3aed] hover:bg-purple-50 transition h-9"
        >
