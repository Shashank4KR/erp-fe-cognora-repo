...
import { Send, Mail, MessageCircle, Bell, CheckCircle } from "lucide-react";
import Card from "@/components/shared/Card";
import type { SummaryCard } from "@/lib/fixtures/communications-announcements-reference-fixture";
...
const Sparkline = ({ data, color }: { data: number[]; color: string }) => {
  ...
  return (
    <svg width={width} height={height} className="mt-1 ml-auto flex-shrink-0">
      <defs>
...
export default function CommunicationSummaryCards({ cards }: CommunicationSummaryCardsProps) {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 mb-6">
      {cards.map((card, idx) => {
        const icons = [
          ...
          <MessageCircle key="sms" className="h-5 w-5" />,
          ...
          <Card key={card.title} className="p-4">
            <div className="flex items-start justify-between mb-2">
              ...
              <div className={`${card.iconBg} p-2 rounded-lg flex-shrink-0`}>
                <span className={card.iconColor}>{icons[idx]}</span>
              </div>
            </div>
