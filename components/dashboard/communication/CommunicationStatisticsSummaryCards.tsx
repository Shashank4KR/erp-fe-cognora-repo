...
import type { StatSummaryCard } from "@/lib/fixtures/communication-statistics-reference-fixture";
...
const Sparkline = ({ data, color }: { data: number[]; color: string }) => {
  ...
  return (
    <svg width={width} height={height} className="mt-1 ml-auto flex-shrink-0">
      ...
      <polygon
        points={areaPoints}
        fill={`url(#stat-grad-${color.replace("#", "")})`}
      />
      <polyline
        points={points}
        fill="none"
...
interface CommunicationStatisticsSummaryCardsProps {
  cards: StatSummaryCard[];
}

export default function CommunicationStatisticsSummaryCards({
  cards,
}: CommunicationStatisticsSummaryCardsProps) {
  return (
