...
export default function RequestsByStatusChart() {
  ...
  const total = STATUS_CHART_SEGMENTS.reduce((sum, s) => sum + s.value, 0);

  return (
    <Card className="p-5">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-base font-semibold text-slate-900">Requests by Status</h2>
      ...
      <div className="flex flex-col sm:flex-row items-center gap-6">
        <div className="relative flex-shrink-0">
          <DonutChart value={TOTAL_REQUESTS_COUNT} label="Total" segments={STATUS_CHART_SEGMENTS} size={140} />
        </div>
        <div className="flex-1 w-full space-y-2.5">
          {STATUS_CHART_SEGMENTS.map((segment) => (
