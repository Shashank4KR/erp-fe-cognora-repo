...
interface HostelManagementPageHeaderProps {
  onAddClick: () => void;
  onMoreOptions: () => void;
}

export default function HostelManagementPageHeader({
  onAddClick,
  onMoreOptions,
}: HostelManagementPageHeaderProps) {
  return (
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
      <div>
        ...
        <nav className="flex items-center gap-1.5 mt-1 text-sm" aria-label="Breadcrumb">
          ...
          <ChevronRight className="w-3.5 h-3.5 text-slate-400" />
          <span className="text-slate-500">Hostel Management</span>
        </nav>
      </div>
