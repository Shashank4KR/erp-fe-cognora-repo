...
import { listStudentAssignments } from "@/lib/services/assignmentService";
  ...
  useEffect(() => {
    const fetchAssignments = async () => {
      try {
        ...
        const data = await listStudentAssignments(token, student.id);
        setAssignments(data || []);
        setError(null);
      } catch (err) {
    ...
    <RoleDashboardLayout config={ROLE_CONFIGS.student}>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-slate-600 mt-1">View your pending assignments and submission deadlines</p>
        </div>
