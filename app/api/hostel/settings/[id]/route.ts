...
export async function GET(
  ...
  try {
    ...
    return await forwardRequest(request, `${getBackendUrl()}/hostel-settings/${id}`);
  } catch {
...
export async function PUT(
  ...
  try {
    const { id } = await params;
    return await forwardRequest(request, `${getBackendUrl()}/hostel-settings/${id}`, "PUT");
  } catch {
    return NextResponse.json({ detail: "Backend is unreachable" }, { status: 502 });
  }
}
...
export async function DELETE(
  ...
  try {
    const { id } = await params;
    return await forwardRequest(request, `${getBackendUrl()}/hostel-settings/${id}`, "DELETE");
  } catch {
    return NextResponse.json({ detail: "Backend is unreachable" }, { status: 502 });
  }
}